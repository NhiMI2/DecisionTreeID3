getwd()
setwd("C:/Users/Dell Latitude/Downloads")
titanic = read.csv("titanic.csv", header = T)
str(titanic)
summary(titanic)
titanic <- subset(titanic, select = -c(cabin, boat,body,home.dest) )
titanic = titanic[complete.cases(titanic),]
newfile = save(titanic,file ="file.csv")
...
Tree_2 = rpart(survived ~ pclass+sex+age+sibsp+parch+fare+embarked, 
               data = mau_xay_dung,method="class", control=rpart.control(cp = 0.006))
prp(Tree_2)
rpart.plot(Tree_2)
...
Tree_3_prune = prune(Tree_3, cp=0.006)
